...
for tf in [0.5, 1.0, 2.0, 4.0]
    eq = FrontEquation(; terms=(CurvatureMotionTerm(beta),), front=mesh, integrator=RK2())
    ...
    println("t=$tf  R_num=$(round(R_num,digits=4))  R_exact=$(round(R_exact,digits=4))")
    mesh = make_circle(R0, N)   # reset for each test
end
